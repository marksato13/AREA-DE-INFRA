...
% TEMPORAL: muestra placeholder hasta que las imágenes estén en img/
% Quitar esta línea cuando se coloquen las imágenes reales
...
\begin{figure}[p]
  \centering
  \includegraphics[width=0.98\linewidth,height=0.82\textheight,keepaspectratio]{CE0312_Arquitectura_Hibrida_Global - FINAL - copia - copia.drawio.bkp.drawio.png}
...
\chapter{Anexo~A --- Arquitectura de vSwitches y Port Groups por Sede}
\label{anex:vswitches}

La arquitectura de vSwitches permite representar dentro de VMware ESXi la separación física y lógica de la red. En este diseño, cada hipervisor utiliza conmutadores virtuales para asociar sus interfaces físicas con redes WAN, enlaces troncales, VLANs internas y grupos de puertos destinados a las máquinas virtuales. Esta organización permite que la infraestructura simulada conserve una estructura similar a una red empresarial real.

El criterio principal aplicado en los hipervisores consiste en separar el tráfico de administración, WAN, LAN interna y servicios por zonas. Para ello, los port groups se configuran con identificadores de VLAN específicos, mientras que los enlaces troncales utilizan VLAN ID 4095 cuando se requiere transportar múltiples VLANs hacia pfSense. De esta forma, pfSense asume el rol de firewall, router inter-VLAN y punto de control del tráfico entre zonas.
...
\begin{xltabular}{\textwidth}{@{}|M{2.5cm}|N{4.2cm}|N{1.4cm}|X|@{}}
\caption{Arquitectura de vSwitches y port groups}
\label{tab:arquitectura_vswitches_portgroups} \\

\arrayrulecolor{upeu_dark}
\rowcolor{upeu_dark}
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small Hipervisor / sede} &
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small vSwitch o port group} &
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small VLAN ID} &
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small Propósito dentro de la arquitectura} \\ \hline
\endfirsthead
...
{\small PG-WAN1-ISP1} &
{\small --} &
{\small Conecta la interfaz WAN1 de pfSense-BORDE hacia el proveedor principal representado por el router Cisco del laboratorio.} \\ \hline

{\small H1 -- Local 2} &
{\small PG-WAN2-ISP2} &
{\small --} &
{\small Conecta la interfaz WAN2 de pfSense-BORDE hacia el enlace secundario simulado mediante FortiGate 40F, permitiendo representar redundancia WAN.} \\ \hline

{\small H1 -- Local 2} &
{\small PG-TRUNK} &
{\small 4095} &
{\small Transporta múltiples VLANs internas hacia pfSense-BORDE. El uso de VLAN ID 4095 permite que pfSense gestione subinterfaces, reglas de firewall y enrutamiento inter-VLAN.} \\ \hline

{\small H1 -- Local 2} &
{\small PG-VLAN30-DMZ} &
{\small 30} &
{\small Aísla los servicios expuestos de forma controlada, principalmente el WAF y el API Gateway. Esta zona actúa como punto intermedio entre Internet, AWS y los servicios internos.} \\ \hline

{\small H2 -- Local 2} &
{\small PG-VLAN10-ADMIN} &
{\small 10} &
{\small Aloja servicios administrativos y de identidad, como Active Directory, DNS y NTP, necesarios para la autenticación y sincronización de la infraestructura.} \\ \hline

{\small H2 -- Local 2} &
{\small PG-VLAN40-SERVICIOS} &
{\small 40} &
{\small Permite alojar servicios internos como servidor de archivos y componentes que requieren comunicación controlada con otras zonas de la red.} \\ \hline

{\small H2 -- Local 2} &
{\small PG-VLAN60-GESTION} &
{\small 60} &
{\small Segmento destinado al monitoreo centralizado (VLAN 60 Gestión), donde se ubican Wazuh Manager, Indexer y Dashboard (H2, 10.10.60.10) para supervisar servidores, red y servicios con integración a AWS CloudWatch.} \\ \hline

{\small H3 -- Local 2} &
{\small PG-VLAN40-SERVICIOS} &
{\small 40} &
{\small Aloja el broker NATS y los microservicios del ERP. Esta zona permite la comunicación interna entre servicios sin exponerlos directamente a usuarios o Internet.} \\ \hline

{\small H3 -- Local 2} &
{\small PG-VLAN50-DATOS} &
{\small 50} &
{\small Segmento reservado para las bases de datos PostgreSQL del ERP. Solo los microservicios autorizados deben comunicarse con esta VLAN.} \\ \hline

{\small H4 -- Local 1} &
{\small PG-WAN1 / PG-WAN2} &
{\small --} &
{\small Interfaces WAN de pfSense-L1: vmnic0=ISP1 (172.17.25.x), vmnic1=ISP2/FortiGate. pfSense-L1 gestiona el Dual-WAN y el túnel VPN IPSec hacia Local 2 (172.16.10.0/30).} \\ \hline

{\small H4 -- Local 1} &
{\small PG-TRUNK} &
{\small 4095} &
{\small Transporta las VLANs internas del Local 1 hacia pfSense-L1 mediante trunk 802.1Q. pfSense crea subinterfaces em2.10/em2.20/em2.40/em2.60/em2.99.} \\ \hline

{\small H4 -- Local 1} &
{\small PG-VLAN10 / PG-VLAN20 / PG-VLAN40 / PG-VLAN60 / PG-VLAN99} &
{\small 10, 20, 40, 60, 99} &
{\small VLANs internas del Local 1: VLAN10=Admin/RODC-DC3 (10.20.10.0/24), VLAN20=Usuarios (10.20.20.0/24), VLAN40=Servicios/FileServer-L1 (10.20.40.0/24), VLAN60=Gestión/Wazuh (10.20.60.0/28), VLAN99=Mgmt ESXi (10.20.99.0/28).} \\ \hline

{\small H5 -- Local 3} &
{\small PG-WAN1 / PG-WAN2} &
{\small --} &
{\small Interfaces WAN de pfSense-L3: vmnic0=ISP1, vmnic1=ISP2/FortiGate. pfSense-L3 gestiona el Dual-WAN y el túnel VPN IPSec hacia Local 2 (172.16.20.0/30).} \\ \hline

{\small H5 -- Local 3} &
{\small PG-TRUNK} &
{\small 4095} &
{\small Transporta las VLANs internas del Local 3 hacia pfSense-L3. Replica la lógica del H4 (Local 1): misma estructura de 5 VLANs con subinterfaces em2.VLAN.} \\ \hline

{\small H5 -- Local 3} &
{\small PG-VLAN10 / PG-VLAN20 / PG-VLAN40 / PG-VLAN60 / PG-VLAN99} &
{\small 10, 20, 40, 60, 99} &
{\small VLANs internas del Local 3: VLAN10=Admin/RODC-DC4 (10.30.10.0/24), VLAN20=Usuarios (10.30.20.0/24), VLAN40=Servicios/FileServer-L3 (10.30.40.0/24), VLAN60=Gestión/Wazuh-agent (10.30.60.0/28), VLAN99=Mgmt ESXi (10.30.99.0/28).} \\ \hline

\end{xltabular}

\normalsize

La configuración de vSwitches y port groups permite que cada sede mantenga una separación clara entre tráfico WAN, tráfico interno y tráfico de administración. En la sede central, esta arquitectura soporta la DMZ, los servicios internos, los microservicios, las bases de datos y el monitoreo. En las sedes remotas, permite conservar una estructura más liviana, pero suficiente para operar localmente y comunicarse con el hub central mediante VPN.


\chapter{Anexo~B --- Inventario Completo de Máquinas Virtuales}
\label{anex:vms}

El inventario de máquinas virtuales permite identificar cómo se distribuyen los servicios de infraestructura, seguridad, aplicaciones, bases de datos y respaldo dentro de los hipervisores definidos para Franco's SAC. En este diseño, las máquinas virtuales no se ubican de forma aleatoria, sino que responden a una separación por función: borde, servicios centrales, backend, datos, respaldo y sedes remotas.

La sede central, representada por el Local 2, concentra la mayor cantidad de máquinas virtuales debido a que aloja el núcleo operativo de la solución. En cambio, el Local 1 y el Local 3 mantienen únicamente los servicios necesarios para operar como sedes remotas livianas, autenticarse localmente y comunicarse de forma segura con la sede central.
...
\begin{xltabular}{\textwidth}{@{}|L{2.3cm}|L{2.9cm}|L{4.6cm}|X|@{}}
\caption{Inventario resumido de máquinas virtuales por hipervisor}
\label{tab:inventario_maquinas_virtuales} \\
...
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small Máquinas virtuales principales} &
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small Función dentro de la arquitectura} \\ \hline
\endfirsthead
...
{\small H1 -- Local 2} &
{\small Borde, CARP HA, DMZ y acceso externo} &
{\small VM-pfSense-BORDE-A (MASTER), VM-pfSense-BORDE-B (BACKUP CARP), VM-WAF (NGINX+ModSec, 10.10.30.5) y VM-API-GW (10.10.30.10).} &
{\small Concentra los componentes de entrada. pfSense CARP HA gestiona Dual-WAN (ISP1+ISP2), failover $<$2s, túneles VPN hub-and-spoke y routing inter-VLAN (Router-on-a-Stick em2.x). WAF = OWASP CRS, rate-limit 100 req/s. Failover a AWS ALB si H1 cae.} \\ \hline

{\small H2 -- Local 2} &
{\small Servicios centralizados} &
{\small VM-AD-DC (DC1, Samba, 10.10.10.10), VM-Wazuh (Manager+Indexer+Dashboard, 10.10.60.10) y VM-FileServer (VLAN 40, 10.10.40.20).} &
{\small Aloja los servicios corporativos: DC1 para autenticación/DNS/LDAP del dominio francos.local (Samba), Wazuh completo en VLAN 60 para monitoreo híbrido con agentes en todas las VMs y CloudWatch, y el servidor de archivos corporativo central.} \\ \hline

{\small H3 -- Local 2} &
{\small Backend ERP y bases de datos} &
{\small VM-NATS-Server (JetStream), VM-auth-ms, VM-person-ms, VM-sales-invent-ms, VM-strategic-ms (Docker restart:always), person-db, sales-invent-db y strategic-db (PostgreSQL 15, VLAN 50).} &
{\small Sostiene el núcleo aplicativo del ERP: NATS JetStream como broker (VLAN 40, puerto 4222), 4 microservicios NestJS 11 con Docker y 3 bases de datos PostgreSQL 15 aisladas en VLAN 50. pg\_dump diario a S3 (22:00) como backup lógico.} \\ \hline

{\small H4 -- Local 1} &
{\small Sede remota liviana (Planta Producción 1)} &
{\small VM-pfSense-L1 (Dual-WAN + VPN L1-L2), VM-AD-RODC-DC3 (Samba RODC, 10.20.10.20) y VM-FileServer-L1 (VLAN 40, 10.20.40.10).} &
{\small Representa la Planta de Producción 1. pfSense-L1 mantiene túnel IPSec IKEv2 AES-256-GCM hacia Local 2 (172.16.10.0/30). RODC DC3 autentica usuarios localmente si la VPN cae. FileServer-L1 en VLAN 40 para acceso local a archivos.} \\ \hline

{\small H5 -- Local 3} &
{\small Sede remota liviana (2ª Planta Producción)} &
{\small VM-pfSense-L3 (Dual-WAN + VPN L2-L3), VM-AD-RODC-DC4 (Samba RODC, 10.30.10.20) y VM-FileServer-L3 (VLAN 40, 10.30.40.10).} &
{\small Replica la logica del H4/Local 1 para la 2a Planta. pfSense-L3 mantiene tunel IPSec hacia Local 2 (172.16.20.0/30). RODC DC4 para autenticacion local. Se documenta con referencia logica de laboratorio para CPU, RAM y almacenamiento.} \\ \hline

\end{xltabular}

\normalsize

Esta distribución permite que la arquitectura mantenga una separación clara de responsabilidades. Los servicios más críticos permanecen en la sede central, mientras que las sedes remotas conservan únicamente los componentes necesarios para garantizar conectividad, autenticación local y continuidad mínima de operación.


\chapter{Anexo~C --- Esquema Completo de Direccionamiento IP}
\label{anex:ip}

El esquema de direccionamiento IP permite ordenar la red de Franco's SAC de acuerdo con la sede, la VLAN y la función de cada segmento. Para mantener una estructura clara, se utiliza una convención jerárquica donde el segundo octeto identifica el local y el tercer octeto representa la VLAN o zona funcional. De esta manera, la administración de rutas, reglas de firewall, monitoreo y documentación se vuelve más simple y trazable.

En este diseño, el Local 2 utiliza el bloque \textbf{10.10.x.x} por ser la sede central; el Local 1 utiliza el bloque \textbf{10.20.x.x}; el Local 3 utiliza el bloque \textbf{10.30.x.x}; y la VPC de AWS se reserva bajo el bloque \textbf{10.40.x.x}. Asimismo, los túneles VPN se ubican en redes \textbf{/30} dentro del rango \textbf{172.16.x.x}, debido a que solo requieren dos direcciones útiles por enlace punto a punto.
...
\begin{xltabular}{\linewidth}{@{}|L{3.2cm}|C{1.3cm}|L{2.8cm}|L{3cm}|L{3cm}|L{3cm}|X|@{}}
\caption{Esquema de direccionamiento IP propuesto}
\label{tab:esquema_direccionamiento_ip} \\

\arrayrulecolor{upeu_dark}
\rowcolor{upeu_dark}
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small Sede / enlace} &
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small VLAN} &
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small Nombre lógico} &
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small Subred} &
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small Máscara} &
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small Gateway / extremo} &
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small Observación} \\ \hline
\endfirsthead
...
{\small 255.255.255.0} &
{\small 10.10.10.1} &
{\small Segmento para servicios administrativos, Active Directory, DNS y administración interna.} \\ \hline
...
{\small 255.255.255.0} &
{\small 10.10.20.1} &
{\small Red destinada a usuarios corporativos, estaciones de trabajo y acceso interno al ERP.} \\ \hline

{\small Local 2 -- Sede Central} &
{\small 30} &
{\small DMZ} &
{\small 10.10.30.0/28} &
{\small 255.255.255.240} &
{\small 10.10.30.1} &
{\small Zona desmilitarizada para WAF y API Gateway. Se usa una subred reducida para limitar la superficie de exposición.} \\ \hline
...
{\small 255.255.255.0} &
{\small 10.10.40.1} &
{\small Segmento para microservicios, NATS, servidor de archivos y servicios internos.} \\ \hline
...
{\small 255.255.255.240} &
{\small 10.10.50.1} &
{\small Red reservada para bases de datos PostgreSQL. El tamaño reducido responde al principio de mínima exposición.} \\ \hline
...
{\small 255.255.255.240} &
{\small 10.10.60.1} &
{\small Segmento destinado a monitoreo híbrido: Wazuh (Manager+Indexer+Dashboard) en H2 (10.10.60.10), agentes Wazuh en todas las VMs, integración CloudWatch, SNMP polling en los 3 locales.} \\ \hline
...
{\small 255.255.255.240} &
{\small 10.10.99.1} &
{\small Red dedicada a la administración de hipervisores y componentes de gestión.} \\ \hline

{\small Local 1 -- Planta Producción 1} &
{\small 10} &
{\small Administración} &
{\small 10.20.10.0/24} &
{\small 255.255.255.0} &
{\small 10.20.10.1} &
{\small RODC DC3 (Samba, 10.20.10.20) + servicios administrativos locales. Sin VPN: RODC autentica localmente.} \\ \hline
...
{\small 255.255.255.0} &
{\small 10.20.20.1} &
{\small Red para estaciones de trabajo y usuarios de producción del Local 1. Acceso al ERP vía VPN.} \\ \hline

{\small Local 1 -- Planta Producción 1} &
{\small 40} &
{\small Servicios / FileServer} &
{\small 10.20.40.0/24} &
{\small 255.255.255.0} &
{\small 10.20.40.1} &
{\small FileServer-L1 (10.20.40.10) para almacenamiento local. VLAN 40 replicada desde L2 para coherencia estructural.} \\ \hline

{\small Local 1 -- Planta Producción 1} &
{\small 60} &
{\small Gestión / Wazuh-agent} &
{\small 10.20.60.0/28} &
{\small 255.255.255.240} &
{\small 10.20.60.1} &
{\small Agentes Wazuh en VMs de H4. Comunicación hacia Wazuh Manager en L2 (10.10.60.10) a través de VPN.} \\ \hline
...
{\small 255.255.255.240} &
{\small 10.20.99.1} &
{\small Administración del hipervisor H4 y componentes internos del Local 1.} \\ \hline

{\small Local 3 -- Segunda Planta} &
{\small 10} &
{\small Administración} &
{\small 10.30.10.0/24} &
{\small 255.255.255.0} &
{\small 10.30.10.1} &
{\small RODC DC4 (Samba, 10.30.10.20) + servicios administrativos locales de la 2ª planta.} \\ \hline
...
{\small 255.255.255.0} &
{\small 10.30.20.1} &
{\small Red para estaciones de trabajo y usuarios operativos del Local 3. Acceso al ERP vía VPN.} \\ \hline
...
{\small 255.255.255.0} &
{\small 10.30.40.1} &
{\small FileServer-L3 (10.30.40.10) para almacenamiento local. Recurso documentado bajo referencia logica de laboratorio.} \\ \hline
...
{\small 255.255.255.240} &
{\small 10.30.60.1} &
{\small Agentes Wazuh en VMs de H5. Comunicación hacia Wazuh Manager en L2 a través de VPN.} \\ \hline
...
{\small 255.255.255.240} &
{\small 10.30.99.1} &
{\small Administración del hipervisor H5 y elementos de gestión del Local 3.} \\ \hline

{\small VPN L1--L2} &
{\small --} &
{\small Túnel IPSec} &
{\small 172.16.10.0/30} &
{\small 255.255.255.252} &
{\small L2: 172.16.10.1 / L1: 172.16.10.2} &
{\small Enlace punto a punto entre el Local 1 y la sede central.} \\ \hline
...
{\small 172.16.20.0/30} &
{\small 255.255.255.252} &
{\small L2: 172.16.20.2 / L3: 172.16.20.1} &
{\small Enlace punto a punto entre la sede central y el Local 3.} \\ \hline
...
{\small 172.16.30.0/30} &
{\small 255.255.255.252} &
{\small L2: 172.16.30.2 / AWS: 172.16.30.1} &
{\small Enlace privado entre la sede central y la VPC de AWS.} \\ \hline

{\small WAN Laboratorio} &
{\small --} &
{\small ISP1 simulado} &
{\small 172.17.25.0/24} &
{\small 255.255.255.0} &
{\small 172.17.25.121} &
{\small Red del laboratorio UPeU utilizada como salida WAN principal en el entorno académico.} \\ \hline

{\small WAN secundaria} &
{\small --} &
{\small ISP2 simulado} &
{\small 192.168.100.0/24} &
{\small 255.255.255.0} &
{\small 192.168.100.1} &
{\small Red asociada al FortiGate 40F para representar un segundo proveedor de Internet.} \\ \hline
...
{\small 255.255.0.0} &
{\small --} &
{\small Espacio reservado para servicios cloud como Route 53, CloudFront, S3, ECR y VPN Gateway.} \\ \hline
...
La planificación del direccionamiento IP facilita el crecimiento ordenado de la infraestructura. Además, permite que las reglas de firewall, las rutas VPN, el monitoreo y la documentación técnica se construyan sobre una convención clara, donde cada sede y cada VLAN pueden identificarse de forma rápida.


\chapter{Anexo~D --- Flujos de Tráfico por Zona}
\label{anex:flujos}

Los flujos de tráfico por zona permiten definir qué comunicaciones deben permitirse, restringirse o bloquearse dentro de la arquitectura de red propuesta. Esta sección es importante porque la segmentación mediante VLANs no solo consiste en separar redes, sino también en controlar de forma explícita cómo se comunican los usuarios, servicios, bases de datos, sedes remotas, DMZ y recursos cloud.

En el diseño de Franco's SAC, el tráfico no debe circular libremente entre todas las VLANs. Por el contrario, cada comunicación debe responder a una necesidad funcional. Por ejemplo, los usuarios pueden acceder al ERP mediante el API Gateway, pero no deben comunicarse directamente con las bases de datos. Del mismo modo, la DMZ puede recibir tráfico controlado desde el exterior, pero no debe tener acceso abierto hacia las zonas internas críticas.
...
\begin{xltabular}{\linewidth}{@{}|L{2.5cm}|L{3.3cm}|L{3.3cm}|L{2.8cm}|C{2.2cm}|X|@{}}
\caption{Flujos de tráfico permitidos, restringidos y bloqueados por zona}
\label{tab:flujos_trafico_zona} \\

\arrayrulecolor{upeu_dark}
\rowcolor{upeu_dark}
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small Flujo} &
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small Origen} &
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small Destino} &
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small Servicio / puerto} &
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small Acción} &
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small Justificación técnica} \\ \hline
\endfirsthead
...
{\small Usuarios al ERP} &
{\small VLAN 20 -- Usuarios} &
{\small VLAN 30 -- DMZ / API Gateway} &
{\small HTTPS 443} &
{\small \textbf{Permitido}} &
{\small Permite que los usuarios internos accedan al ERP mediante un punto de entrada controlado, sin comunicarse directamente con microservicios ni bases de datos.} \\ \hline

{\small Acceso externo controlado} &
{\small Internet / AWS CloudFront} &
{\small VLAN 30 -- WAF} &
{\small HTTPS 443} &
{\small \textbf{Controlado}} &
{\small El tráfico externo debe ingresar primero por una capa de protección web. El WAF filtra solicitudes antes de enviarlas hacia el API Gateway.} \\ \hline

{\small WAF hacia API Gateway} &
{\small VLAN 30 -- WAF} &
{\small VLAN 30 -- API Gateway} &
{\small HTTP/HTTPS} &
{\small \textbf{Permitido}} &
{\small Permite que el WAF reenvíe únicamente tráfico validado hacia el API Gateway, manteniendo la exposición limitada dentro de la DMZ.} \\ \hline

{\small API hacia servicios} &
{\small VLAN 30 -- API Gateway} &
{\small VLAN 40 -- Microservicios / NATS} &
{\small TCP 4222 / tráfico interno autorizado} &
{\small \textbf{Permitido}} &
{\small Habilita la comunicación del API Gateway con la capa de servicios y mensajería, necesaria para el funcionamiento del ERP basado en microservicios.} \\ \hline

{\small Servicios hacia datos} &
{\small VLAN 40 -- Microservicios} &
{\small VLAN 50 -- PostgreSQL} &
{\small TCP 5432} &
{\small \textbf{Permitido}} &
{\small Permite que únicamente los microservicios autorizados consulten o registren información en las bases de datos correspondientes.} \\ \hline

{\small Usuarios hacia base de datos} &
{\small VLAN 20 -- Usuarios} &
{\small VLAN 50 -- Datos} &
{\small Cualquier puerto} &
{\small \textbf{Bloqueado}} &
{\small Los usuarios no deben acceder directamente a las bases de datos. Toda operación debe pasar por la capa de aplicación para mantener seguridad, trazabilidad y control.} \\ \hline

{\small Usuarios hacia servicios internos} &
{\small VLAN 20 -- Usuarios} &
{\small VLAN 40 -- Servicios} &
{\small Cualquier puerto directo} &
{\small \textbf{Bloqueado}} &
{\small Evita que los usuarios consuman microservicios, broker NATS o servicios internos sin pasar por el API Gateway.} \\ \hline

{\small Administración de dominio} &
{\small VLAN 10 -- Administración} &
{\small Controladores AD / DNS / NTP} &
{\small DNS 53, Kerberos 88, LDAP 389, NTP 123} &
{\small \textbf{Permitido}} &
{\small Permite autenticación, resolución de nombres y sincronización horaria para usuarios y equipos autorizados dentro del dominio corporativo.} \\ \hline

{\small Monitoreo técnico} &
{\small VLAN 60 -- Gestión} &
{\small Servidores, firewalls, VMs y dispositivos} &
{\small SNMP, ICMP, agentes Wazuh} &
{\small \textbf{Permitido}} &
{\small Permite supervisar disponibilidad, latencia, consumo de recursos y estado de servicios críticos desde la plataforma de monitoreo.} \\ \hline

{\small Gestión de hipervisores} &
{\small VLAN 60 -- Gestión} &
{\small VLAN 99 -- Management ESXi} &
{\small HTTPS 443 / SSH 22} &
{\small \textbf{Controlado}} &
{\small La administración de ESXi debe limitarse al personal técnico autorizado, evitando exposición desde redes de usuarios o sedes no administrativas.} \\ \hline

{\small Locales remotos al ERP} &
{\small Local 1 y Local 3} &
{\small Local 2 -- API Gateway / servicios centrales} &
{\small HTTPS 443 vía VPN} &
{\small \textbf{Permitido}} &
{\small Permite que las plantas de producción accedan al ERP centralizado mediante túneles IPSec, manteniendo cifrado y control desde la sede central.} \\ \hline

{\small RODC hacia AD central} &
{\small RODC Local 1 / RODC Local 3} &
{\small AD principal Local 2} &
{\small DNS, LDAP, Kerberos, replicación AD} &
{\small \textbf{Permitido}} &
{\small Permite autenticación local en sedes remotas y sincronización controlada con el dominio central a través de la VPN.} \\ \hline

{\small Locales remotos hacia datos} &
{\small Local 1 y Local 3} &
{\small VLAN 50 -- Bases de datos} &
{\small TCP 5432 directo} &
{\small \textbf{Bloqueado}} &
{\small Las sedes remotas no deben conectarse directamente a las bases de datos. El acceso debe realizarse por el ERP y sus microservicios.} \\ \hline

{\small Sede central hacia AWS} &
{\small Local 2 -- pfSense-BORDE} &
{\small AWS VPC / S3 / ECR} &
{\small VPN IPSec / HTTPS 443} &
{\small \textbf{Permitido}} &
{\small Habilita respaldo, publicación de servicios, repositorio de imágenes Docker y comunicación privada con recursos cloud.} \\ \hline

{\small Internet hacia redes internas} &
{\small Internet} &
{\small VLAN 10, 20, 40, 50, 60 y 99} &
{\small Cualquier puerto} &
{\small \textbf{Bloqueado}} &
{\small Ninguna red interna crítica debe exponerse directamente a Internet. El único punto controlado de entrada debe ser la DMZ.} \\ \hline

{\small DMZ hacia datos} &
{\small VLAN 30 -- DMZ} &
{\small VLAN 50 -- Datos} &
{\small TCP 5432 directo} &
{\small \textbf{Bloqueado}} &
{\small La DMZ no debe consultar bases de datos directamente. Esta restricción reduce el impacto ante una posible vulneración de un servicio expuesto.} \\ \hline
...
La matriz de flujos permite transformar la segmentación lógica en reglas concretas de seguridad. En este diseño, pfSense cumple un rol central porque controla el tráfico entre VLANs, sedes remotas, DMZ, servicios internos y AWS. Bajo este enfoque, cada comunicación permitida responde a una necesidad operativa, mientras que los accesos directos hacia zonas críticas se restringen o bloquean por principio de mínimo privilegio.


\chapter{Anexo~E --- Cálculos de Subnetting y Subredes VPN hub-and-spoke}
\label{anex:subnetting}

Los cálculos de subnetting permiten validar que las subredes definidas para Franco's SAC sean coherentes con la cantidad de dispositivos esperados, la criticidad de cada zona y el principio de mínima superficie de exposición. En este bloque no se vuelve a listar todo el direccionamiento IP, ya que este fue detallado previamente, sino que se justifica el tamaño de red utilizado para cada tipo de segmento.

El diseño emplea tres tamaños principales de subred: \textbf{/24} para zonas con crecimiento operativo, \textbf{/28} para segmentos críticos o reducidos, y \textbf{/30} para enlaces punto a punto entre sedes y AWS. Esta combinación permite equilibrar capacidad, orden y seguridad.
...
\begin{xltabular}{\textwidth}{@{}|L{3.2cm}|N{1.5cm}|N{2.2cm}|N{2.4cm}|X|@{}}
\caption{Criterio de dimensionamiento de subredes}
\label{tab:criterio_dimensionamiento_subredes} \\

\arrayrulecolor{upeu_dark}
\rowcolor{upeu_dark}
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small Tipo de segmento} &
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small Prefijo} &
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small Hosts útiles} &
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small Hosts operativos} &
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small Justificación de uso} \\ \hline
\endfirsthead
...
{\small Administración, usuarios y servicios} &
{\small /24} &
{\small 254} &
{\small 253} &
{\small Se utiliza en segmentos con mayor posibilidad de crecimiento, como usuarios, administración y servicios internos. Se reserva una dirección para el gateway de pfSense, quedando 253 direcciones operativas para equipos, servidores o servicios.} \\ \hline

{\small DMZ, datos, gestión y management} &
{\small /28} &
{\small 14} &
{\small 13} &
{\small Se aplica en zonas críticas o de tamaño reducido. Al reservar una dirección para el gateway, quedan 13 direcciones operativas, suficientes para los componentes definidos y alineadas con la reducción de superficie de exposición.} \\ \hline

{\small Enlaces VPN punto a punto} &
{\small /30} &
{\small 2} &
{\small 2} &
{\small Se emplea para túneles entre sedes y AWS, donde solo se requiere una dirección por extremo del enlace. Esto evita desperdicio de direcciones y mantiene una asignación clara para cada túnel.} \\ \hline

\end{xltabular}

\normalsize

Para validar el cálculo de hosts disponibles se considera la relación general entre bits de host y direcciones utilizables. En redes IPv4, dos direcciones se reservan para la dirección de red y broadcast; por ello, los hosts útiles se calculan restando dos direcciones al total del bloque.
...
\begin{xltabular}{\textwidth}{@{}|N{1.6cm}|N{2cm}|N{2.2cm}|N{2.3cm}|X|@{}}
\caption{Validación matemática de prefijos utilizados}
\label{tab:validacion_prefijos_subnetting} \\

\arrayrulecolor{upeu_dark}
\rowcolor{upeu_dark}
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small Prefijo} &
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small Bits de host} &
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small Total direcciones} &
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small Hosts útiles} &
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small Aplicación dentro del diseño} \\ \hline
\endfirsthead
...
{\small \(2^8 = 256\)} &
{\small 254} &
{\small Segmentos de administración, usuarios y servicios, donde se requiere margen de crecimiento para equipos, estaciones de trabajo, servidores o componentes internos.} \\ \hline

{\small /28} &
{\small 4} &
{\small \(2^4 = 16\)} &
{\small 14} &
{\small Segmentos reducidos como DMZ, datos, gestión y management ESXi. En la práctica se reserva una IP para el gateway, quedando 13 direcciones asignables a hosts.} \\ \hline

{\small /30} &
{\small 2} &
{\small \(2^2 = 4\)} &
{\small 2} &
{\small Túneles VPN punto a punto, donde se necesitan únicamente dos direcciones útiles: una para cada extremo del enlace.} \\ \hline

\end{xltabular}

\normalsize

Desde el punto de vista técnico, las subredes \textbf{/24} permiten crecimiento operativo sin rediseñar la red a corto plazo. En cambio, las subredes \textbf{/28} se aplican en zonas donde no se espera una gran cantidad de hosts y donde conviene limitar la cantidad de direcciones disponibles. Finalmente, las subredes \textbf{/30} son adecuadas para enlaces punto a punto porque asignan exactamente dos direcciones útiles.

Esta decisión evita dos extremos: sobredimensionar todas las VLANs con redes grandes o reducir excesivamente segmentos que sí requieren crecimiento. Por ello, el subnetting propuesto mantiene coherencia entre capacidad, seguridad y administración.

\section{Subredes VPN hub-and-spoke}

Las subredes VPN permiten interconectar las sedes de Franco's SAC y la nube de AWS mediante enlaces punto a punto. Como la conectividad interlocal ya fue descrita en el diseño topológico, en este bloque se aborda únicamente desde el enfoque de segmentación y subnetting.

El modelo utilizado es \textit{hub-and-spoke}, donde el Local 2 actúa como sede central o concentrador. Bajo esta lógica, el Local 1, el Local 3 y AWS no se conectan entre sí de forma directa, sino que establecen sus túneles hacia la sede central. Para estos enlaces se utilizan subredes \textbf{/30}, debido a que cada túnel requiere solo dos direcciones útiles: una para cada extremo.
...
\begin{xltabular}{\textwidth}{@{}|L{2.4cm}|L{2.7cm}|L{2.9cm}|L{2.9cm}|X|@{}}
\caption{Subredes VPN definidas para la arquitectura hub-and-spoke}
\label{tab:subredes_vpn_hub_spoke} \\

\arrayrulecolor{upeu_dark}
\rowcolor{upeu_dark}
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small Enlace VPN} &
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small Subred} &
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small Extremo sede central} &
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small Extremo remoto} &
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small Propósito de segmentación} \\ \hline
\endfirsthead
...
{\small 172.16.10.1} &
{\small 172.16.10.2} &
{\small Permite conectar la primera planta de producción con la sede central, manteniendo un enlace dedicado para el tráfico entre la red 10.20.0.0/16 y los servicios ubicados en 10.10.0.0/16.} \\ \hline

{\small Local 2 -- Local 3} &
{\small 172.16.20.0/30} &
{\small 172.16.20.2} &
{\small 172.16.20.1} &
{\small Permite integrar la segunda planta de producción con la sede central mediante un enlace punto a punto independiente, evitando mezclar su tráfico con el túnel del Local 1.} \\ \hline

{\small Local 2 -- AWS} &
{\small 172.16.30.0/30} &
{\small 172.16.30.2} &
{\small 172.16.30.1} &
{\small Permite conectar la sede central con la VPC de AWS, separando el tráfico cloud del tráfico interlocal y manteniendo una ruta específica para servicios de respaldo, publicación o repositorios.} \\ \hline

\end{xltabular}

\normalsize

El uso de subredes \textbf{/30} resulta adecuado para este escenario porque evita asignar bloques más grandes a enlaces que solo necesitan dos direcciones. Además, cada túnel conserva su propio segmento, lo que facilita la administración de rutas, reglas de firewall, monitoreo y diagnóstico de fallas.

Con esta distribución, la segmentación VPN queda alineada con la arquitectura general: Local 2 concentra el control de comunicación entre sedes, mientras que los locales remotos acceden a los servicios centrales sin requerir enlaces directos entre ellos.


\chapter{Anexo~F --- Configuración Inter-VLAN Routing y Trunk IEEE 802.1Q}
\label{anex:routing}

El enrutamiento inter-VLAN permite que las redes segmentadas puedan comunicarse de forma controlada cuando existe una necesidad operativa. En el diseño de Franco's SAC, esta función se asigna a pfSense, el cual actúa como gateway lógico de cada VLAN y como punto de control del tráfico entre zonas.

La técnica utilizada corresponde al enfoque \textit{Router-on-a-Stick}. Bajo este modelo, una interfaz física o virtual transporta múltiples VLANs etiquetadas mediante IEEE 802.1Q, mientras que pfSense crea subinterfaces asociadas a cada VLAN. Cada subinterfaz funciona como puerta de enlace del segmento correspondiente y permite aplicar reglas de firewall específicas según el origen, destino y tipo de servicio.

Este enfoque evita que el tráfico entre VLANs circule libremente. Por ejemplo, una estación de usuarios puede acceder al ERP mediante la DMZ, pero no debería conectarse directamente a las bases de datos. Del mismo modo, la red de gestión puede monitorear servicios, pero no debe convertirse en una red de acceso general para usuarios.
...
\begin{xltabular}{\textwidth}{@{}|L{2.7cm}|L{3cm}|L{3.4cm}|X|@{}}
\caption{Rol de pfSense en el enrutamiento inter-VLAN}
\label{tab:rol_pfsense_intervlan} \\

\arrayrulecolor{upeu_dark}
\rowcolor{upeu_dark}
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small Sede} &
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small Firewall / gateway} &
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small VLANs enrutadas} &
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small Función dentro del diseño} \\ \hline
\endfirsthead
...
{\small Local 2 -- Sede Central} &
{\small pfSense-BORDE} &
{\small VLAN 10, 20, 30, 40, 50, 60 y 99} &
{\small Actúa como gateway principal de la sede central. Controla el tráfico entre usuarios, DMZ, servicios, datos, gestión y administración de hipervisores. También concentra la salida WAN y los túneles VPN hacia sedes remotas y AWS.} \\ \hline

{\small Local 1 -- Planta de Producción 1} &
{\small pfSense-L1} &
{\small VLAN 10, 20, 60 y 99} &
{\small Enruta las VLANs internas de la sede remota y controla el acceso hacia la sede central mediante VPN. Su función es mantener una segmentación local simple, sin alojar servicios críticos expuestos.} \\ \hline

{\small Local 3 -- Segunda Planta de Producción} &
{\small pfSense-L3} &
{\small VLAN 10, 20, 60 y 99} &
{\small Replica la lógica de segmentación del Local 1, permitiendo comunicación controlada hacia los servicios centralizados sin incorporar DMZ, microservicios ni bases de datos locales.} \\ \hline

\end{xltabular}

\normalsize

La creación de subinterfaces permite que pfSense identifique el tráfico de cada VLAN y aplique reglas diferenciadas. En la sede central, por ejemplo, las subinterfaces de pfSense-BORDE se asocian a las VLANs críticas de la arquitectura, mientras que en las sedes remotas se mantiene solo el conjunto mínimo de VLANs necesarias para operación y gestión.
...
\begin{xltabular}{\textwidth}{@{}|L{2.6cm}|N{1.5cm}|L{3cm}|X|@{}}
\caption{Subinterfaces lógicas consideradas en pfSense}
\label{tab:subinterfaces_pfsense} \\

\arrayrulecolor{upeu_dark}
\rowcolor{upeu_dark}
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small Firewall} &
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small VLAN} &
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small Subinterfaz} &
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small Uso dentro del routing inter-VLAN} \\ \hline
\endfirsthead
...
{\small 10} &
{\small em2.10} &
{\small Gateway de la red administrativa de la sede central. Gestiona tráfico asociado a identidad, DNS, NTP y administración interna.} \\ \hline

{\small pfSense-BORDE} &
{\small 20} &
{\small em2.20} &
{\small Gateway de usuarios corporativos. Desde esta VLAN se controla el acceso hacia servicios publicados y recursos internos autorizados.} \\ \hline

{\small pfSense-BORDE} &
{\small 30} &
{\small em2.30} &
{\small Gateway de la DMZ. Controla el acceso hacia WAF, API Gateway y comunicación limitada hacia la capa de servicios.} \\ \hline

{\small pfSense-BORDE} &
{\small 40} &
{\small em2.40} &
{\small Gateway de servicios internos, microservicios, broker de mensajería y componentes del ERP.} \\ \hline

{\small pfSense-BORDE} &
{\small 50} &
{\small em2.50} &
{\small Gateway de la zona de datos. Su acceso debe restringirse únicamente a servicios autorizados.} \\ \hline

{\small pfSense-BORDE} &
{\small 60} &
{\small em2.60} &
{\small Gateway de gestión y monitoreo, utilizado para supervisión técnica de servidores, red y servicios.} \\ \hline

{\small pfSense-BORDE} &
{\small 99} &
{\small em2.99} &
{\small Gateway de administración de hipervisores ESXi y componentes de gestión de virtualización.} \\ \hline

{\small pfSense-L1 / pfSense-L3} &
{\small 10, 20, 40, 60 y 99} &
{\small em2.10, em2.20, em2.40, em2.60, em2.99} &
{\small Subinterfaces utilizadas por las sedes remotas para separar administración, usuarios, servicios/FileServer local, gestión y administración del hipervisor.} \\ \hline

\end{xltabular}

\normalsize

El routing inter-VLAN no implica permitir toda comunicación entre segmentos. Su función es habilitar la posibilidad técnica de comunicación, pero la decisión final depende de las reglas de firewall aplicadas en pfSense. Por ello, el diseño mantiene una política restrictiva: todo tráfico no justificado debe permanecer bloqueado, y solo se habilitan los flujos necesarios para operación, monitoreo, autenticación y acceso controlado al ERP.

\section{Configuración trunk IEEE 802.1Q}

La configuración trunk permite transportar múltiples VLANs a través de un mismo enlace físico o virtual. En el diseño de Franco's SAC, este mecanismo es necesario porque pfSense debe recibir varias VLANs sobre una sola interfaz y, a partir de ellas, crear subinterfaces lógicas para actuar como gateway y firewall de cada segmento.

El estándar \textbf{IEEE 802.1Q} permite etiquetar las tramas Ethernet con el identificador de VLAN correspondiente. De esta manera, el switch, el hipervisor ESXi y pfSense pueden reconocer a qué red lógica pertenece cada paquete, evitando mezclar tráfico de usuarios, administración, DMZ, servicios, datos, gestión o administración de hipervisores.

En este bloque se describe el trunk como mecanismo de transporte. Las reglas de comunicación entre VLANs no se definen aquí, ya que pertenecen a la política de segmentación y control de tráfico tratada en los bloques posteriores.
...
\begin{xltabular}{\textwidth}{@{}|L{3.3cm}|L{3.6cm}|X|@{}}
\caption{Elementos considerados en la configuración trunk IEEE 802.1Q}
\label{tab:configuracion_trunk_8021q} \\

\arrayrulecolor{upeu_dark}
\rowcolor{upeu_dark}
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small Elemento} &
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small Configuración considerada} &
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small Propósito dentro del diseño} \\ \hline
\endfirsthead
...
{\small ESXi Port Group Trunk} &
{\small VLAN ID 4095} &
{\small Permite que el port group transporte múltiples VLANs etiquetadas hacia la máquina virtual pfSense. En VMware ESXi, el valor 4095 se utiliza para habilitar el paso de tráfico VLAN etiquetado.} \\ \hline

{\small Interfaz LAN de pfSense} &
{\small Interfaz troncal asociada a em2} &
{\small Recibe el tráfico etiquetado de las VLANs y permite crear subinterfaces como em2.10, em2.20, em2.30, em2.40, em2.50, em2.60 y em2.99, según corresponda a cada sede.} \\ \hline

{\small Subinterfaces pfSense} &
{\small em2.VLAN\_ID} &
{\small Cada subinterfaz representa una VLAN específica y actúa como gateway lógico del segmento. Esto permite aplicar reglas diferenciadas de firewall por zona.} \\ \hline

{\small Puerto Cisco Switch} &
{\small Puerto en modo trunk 802.1Q} &
{\small Transporta las VLANs permitidas entre el hipervisor, pfSense y los demás segmentos de la red. Su función es mantener el tráfico etiquetado sin convertirlo en una red plana.} \\ \hline

{\small VLANs permitidas} &
{\small Según sede} &
{\small En la sede central se transportan las VLANs 10, 20, 30, 40, 50, 60 y 99. En las sedes remotas se transportan las VLANs 10, 20, 40, 60 y 99.} \\ \hline

{\small Native VLAN} &
{\small No utilizada como criterio de seguridad} &
{\small Se evita depender de una VLAN nativa para reducir riesgos asociados a configuraciones ambiguas o ataques de salto de VLAN. Todo tráfico debe viajar etiquetado.} \\ \hline

\end{xltabular}

\normalsize

La configuración trunk permite que la segmentación lógica definida en las VLANs pueda operar sobre una infraestructura física y virtual compartida. Sin este mecanismo, sería necesario disponer de una interfaz independiente por cada VLAN, lo que aumentaría la complejidad del diseño y dificultaría su administración en el entorno de laboratorio.
...
\begin{xltabular}{\textwidth}{@{}|L{3cm}|L{3.3cm}|X|@{}}
\caption{VLANs transportadas por trunk según sede}
\label{tab:vlans_trunk_por_sede} \\

\arrayrulecolor{upeu_dark}
\rowcolor{upeu_dark}
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small Sede} &
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small VLANs transportadas} &
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small Criterio de transporte} \\ \hline
\endfirsthead
...
{\small Local 2 -- Sede Central} &
{\small 10, 20, 30, 40, 50, 60 y 99} &
{\small Se transportan todas las VLANs críticas porque la sede central aloja usuarios, DMZ, servicios, bases de datos, monitoreo y administración de hipervisores.} \\ \hline

{\small Local 1 -- Planta de Producción 1} &
{\small 10, 20, 40, 60 y 99} &
{\small Se transportan las VLANs necesarias para administración local, usuarios, FileServer local (VLAN 40), gestión técnica y administración del hipervisor.} \\ \hline

{\small Local 3 -- Segunda Planta de Producción} &
{\small 10, 20, 40, 60 y 99} &
{\small Mantiene la misma lógica del Local 1, incluyendo VLAN 40 para FileServer-L3. No se transportan VLANs exclusivas de la sede central como DMZ, microservicios o datos.} \\ \hline

\end{xltabular}

\normalsize

Con esta configuración, el trunk IEEE 802.1Q funciona como el mecanismo que sostiene la segmentación VLAN sobre la infraestructura virtualizada. pfSense recibe el tráfico etiquetado, identifica cada VLAN mediante sus subinterfaces y aplica el control correspondiente. De esta forma, la red mantiene separación lógica sin requerir múltiples enlaces físicos independientes para cada segmento.


\chapter{Anexo~G --- Plan de Respaldo y Procedimiento de Recuperación}
\label{anex:respaldo}

El plan de respaldo y recuperación define cómo Franco's SAC puede restaurar servicios, configuraciones y datos ante una falla técnica. A diferencia de la redundancia, que busca mantener un servicio disponible durante una interrupción, el respaldo permite recuperar el estado operativo cuando el servicio ya fue afectado.

En este diseño, la recuperación se organiza por niveles. No todos los componentes requieren el mismo tipo de respaldo ni la misma frecuencia. Las bases de datos necesitan un mecanismo más cercano al tiempo real, mientras que las máquinas virtuales, configuraciones de firewall y servicios de monitoreo pueden recuperarse mediante snapshots, exportaciones o respaldos programados.
...
\begin{xltabular}{\textwidth}{@{}|N{1.1cm}|L{3cm}|L{2cm}|L{2.8cm}|X|@{}}
\caption{Niveles de respaldo considerados para la infraestructura}
\label{tab:niveles_respaldo_infraestructura} \\

\arrayrulecolor{upeu_dark}
\rowcolor{upeu_dark}
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small Nivel} &
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small Tipo de respaldo} &
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small Frecuencia} &
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small Destino} &
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small Propósito de recuperación} \\ \hline
\endfirsthead
...
{\small N1} &
{\small Backup a AWS RDS Multi-AZ + pg\_dump a S3} &
{\small Continua (RDS) / Diario 22:00 (pg\_dump)} &
{\small AWS RDS Multi-AZ (nube)} &
{\small Reducir la pérdida de datos ante una falla de la base principal. RDS Multi-AZ proporciona failover automático; pg\_dump a S3 cubre recuperación ante corrupción o escenario severo.} \\ \hline

{\small N2} &
{\small Snapshot de máquinas virtuales críticas} &
{\small Diario o antes de cambios relevantes} &
{\small Datastore local del hipervisor} &
{\small Permitir reversión rápida de VMs ante errores de configuración, fallas de sistema o cambios fallidos.} \\ \hline

{\small N3} &
{\small Respaldo lógico de base de datos mediante pg\_dump} &
{\small Diario} &
{\small Almacenamiento externo o AWS S3} &
{\small Contar con una copia lógica independiente de la réplica, útil ante corrupción, eliminación accidental o falla severa del entorno local.} \\ \hline

{\small N4} &
{\small Exportación OVF/OVA de VMs críticas} &
{\small Semanal} &
{\small Repositorio externo o AWS S3} &
{\small Facilitar la reconstrucción de máquinas virtuales completas en otro host disponible cuando el snapshot local no sea suficiente.} \\ \hline

{\small N5} &
{\small Backup de configuración pfSense} &
{\small Después de cambios y de forma programada} &
{\small Repositorio seguro de configuraciones} &
{\small Restaurar rápidamente reglas, interfaces, gateways, VPNs, NAT y configuraciones críticas del firewall.} \\ \hline

{\small N6} &
{\small Backup de configuración de monitoreo} &
{\small Semanal o después de ajustes relevantes} &
{\small Servidor de respaldo o repositorio externo} &
{\small Recuperar dashboards, alertas, agentes y parámetros de supervisión ante caída del servidor de monitoreo.} \\ \hline

\end{xltabular}

\normalsize

El flujo de recuperación se plantea como un procedimiento operativo. Esta forma permite que el equipo técnico pueda seguir una secuencia clara sin depender únicamente de diagramas. Además, facilita documentar responsables, tiempos de atención y evidencias posteriores al incidente.

\subsection{Procedimiento general de recuperación ante falla de VM crítica}

\begin{enumerate}
    \item \textbf{Detectar la falla.} La alerta puede provenir de Wazuh/CloudWatch, del usuario final, de una revisión manual en ESXi o de la indisponibilidad de un servicio del ERP.

    \item \textbf{Identificar el componente afectado.} Se valida si la falla corresponde a conectividad, firewall, máquina virtual, servicio, base de datos, almacenamiento o hipervisor.

    \item \textbf{Evaluar el alcance del incidente.} Se determina si la interrupción afecta a un servicio aislado, a una VLAN, a una sede completa o a varios componentes dependientes.

    \item \textbf{Intentar recuperación controlada del servicio.} Si el sistema operativo o la VM continúan accesibles, se intenta reiniciar el servicio afectado o el contenedor correspondiente. En el caso de microservicios, se prioriza el reinicio automático si está configurado.

    \item \textbf{Restaurar desde snapshot si la falla persiste.} Cuando el reinicio no resuelve el problema, se revierte la VM al snapshot más reciente válido. Antes de hacerlo, se debe verificar que el snapshot corresponda a un estado estable.

    \item \textbf{Activar servicio en espera si aplica.} Si la falla afecta al API Gateway o WAF con instancia de respaldo en H2, se procede con la activación controlada del servicio en espera y la validación de conectividad.

    \item \textbf{Recuperar base de datos según el escenario.} Si la base principal falla, se evalúa la réplica disponible. Si la réplica no es suficiente o existe corrupción, se restaura desde el respaldo lógico más reciente.

    \item \textbf{Validar la operación del servicio.} Se comprueba acceso al ERP, resolución DNS, autenticación, conectividad entre VLANs autorizadas y disponibilidad de servicios dependientes.

    \item \textbf{Documentar el incidente.} Se registra la causa probable, acciones ejecutadas, tiempo de recuperación, evidencia técnica y recomendaciones de mejora.

    \item \textbf{Revisar medidas preventivas.} Después de la recuperación, se analiza si corresponde ajustar monitoreo, frecuencia de backup, reglas de firewall, capacidad de almacenamiento o procedimientos de cambio.
\end{enumerate}
...
\begin{xltabular}{\textwidth}{@{}|L{3.2cm}|L{4cm}|X|@{}}
\caption{Criterios de recuperación según tipo de incidente}
\label{tab:criterios_recuperacion_incidente} \\

\arrayrulecolor{upeu_dark}
\rowcolor{upeu_dark}
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small Escenario} &
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small Acción principal} &
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small Criterio técnico} \\ \hline
\endfirsthead
...
{\small Caída de servicio o contenedor} &
{\small Reinicio automático o reinicio manual controlado.} &
{\small Se utiliza cuando la VM permanece estable y la falla se limita a un proceso específico.} \\ \hline

{\small Error de configuración en VM} &
{\small Reversión desde snapshot reciente.} &
{\small Se aplica cuando el sistema queda inestable después de cambios, actualizaciones o configuraciones erróneas.} \\ \hline

{\small Falla de VM crítica} &
{\small Restauración desde snapshot o despliegue desde OVF.} &
{\small Se aplica cuando la VM no inicia, presenta corrupción o no puede recuperarse mediante reinicio.} \\ \hline

{\small Falla de base de datos principal} &
{\small Uso de réplica o restauración desde pg\_dump.} &
{\small La réplica reduce el RPO operativo. El backup lógico se reserva para escenarios de corrupción, eliminación accidental o falla severa.} \\ \hline

{\small Falla de pfSense} &
{\small Restauración de VM y carga de configuración respaldada.} &
{\small Permite recuperar interfaces, reglas, NAT, gateways y VPNs sin reconstruir manualmente toda la configuración.} \\ \hline

{\small Falla de monitoreo} &
{\small Restauración desde snapshot o backup de configuración.} &
{\small No detiene el ERP, pero debe recuperarse para restablecer visibilidad sobre disponibilidad, rendimiento y alertas.} \\ \hline

\end{xltabular}

\normalsize

El plan de recuperación debe validarse periódicamente. No basta con generar respaldos; también es necesario comprobar que puedan restaurarse. Por ello, se recomienda realizar pruebas controladas de restauración, verificar la integridad de los dumps de base de datos, revisar el espacio disponible en datastores y confirmar que los backups de configuración de pfSense correspondan a versiones recientes y funcionales.

En síntesis, este bloque establece una estrategia de recuperación por niveles. Los mecanismos de réplica reducen la pérdida de datos, los snapshots permiten recuperación rápida ante fallas menores, los OVF facilitan reconstrucción ante incidentes mayores y los backups externos proporcionan una capa adicional de seguridad frente a eventos severos.


\chapter{Anexo~H --- Matriz de Trazabilidad Diseño--Estándar}
\label{anex:trazabilidad}

La matriz de trazabilidad permite relacionar cada decisión técnica relevante del diseño con una referencia normativa o técnica. Su propósito es demostrar que la propuesta de conectividad no se basa únicamente en criterios operativos, sino que mantiene correspondencia con estándares de cableado, Ethernet, VLANs, seguridad, continuidad y buenas prácticas de infraestructura.

Esta matriz no reemplaza los bloques técnicos anteriores. Su función es consolidar la evidencia normativa, indicando dónde se encuentra desarrollado cada elemento dentro del documento general y cuál es su estado dentro del alcance académico.
...
\begin{xltabular}{\linewidth}{@{}|N{1.4cm}|L{5.2cm}|L{4.4cm}|L{3.5cm}|L{2.1cm}|@{}}
\caption{Matriz de trazabilidad entre elementos del diseño y estándares aplicables}
\label{tab:matriz_trazabilidad_diseno_estandar} \\

\arrayrulecolor{upeu_dark}
\rowcolor{upeu_dark}
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small Código} &
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small Elemento del diseño} &
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small Estándar o referencia asociada} &
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small Evidencia interna} &
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small Estado} \\ \hline
\endfirsthead
...
{\small T-01} &
{\small Enlaces trunk entre ESXi, pfSense y switch de distribución.} &
{\small IEEE 802.1Q} &
{\small CE0312 -- Topología lógica y física\newline CE0313 -- Trunk IEEE 802.1Q} &
{\small CUMPLE} \\ \hline

{\small T-02} &
{\small Segmentación por VLANs para administración, usuarios, DMZ, servicios, datos, gestión y management ESXi.} &
{\small IEEE 802.1Q\newline ISO/IEC 27001 A.13} &
{\small CE0313 -- Diseño de VLANs por sede} &
{\small CUMPLE} \\ \hline

{\small T-03} &
{\small DMZ exclusiva en la sede central para servicios expuestos.} &
{\small NIST SP 800-53 SC-7\newline IEEE 802.1Q\newline \cite{nist80053,ieee8021q}} &
{\small CE0313 -- Diseño de la DMZ} &
{\small CUMPLE} \\ \hline

{\small T-04} &
{\small Cableado UTP Cat6 para conexiones principales de red.} &
{\small TIA/EIA-568\newline ISO/IEC 11801} &
{\small CE0312 -- Inventario físico\newline CE0315 -- Cableado estructurado} &
{\small CUMPLE} \\ \hline

{\small T-05} &
{\small Uso de conectores RJ45 bajo terminación estándar T568B.} &
{\small TIA/EIA-568} &
{\small CE0315 -- Cableado estructurado} &
{\small CUMPLE} \\ \hline

{\small T-06} &
{\small Interfaces Ethernet Gigabit para enlaces internos de mayor capacidad.} &
{\small IEEE 802.3ab} &
{\small CE0312 -- Inventario de dispositivos físicos} &
{\small CUMPLE} \\ \hline

{\small T-07} &
{\small Enlaces Fast Ethernet presentes en componentes de borde o laboratorio.} &
{\small IEEE 802.3u} &
{\small CE0312 -- Inventario de dispositivos físicos} &
{\small PARCIAL} \\ \hline

{\small T-08} &
{\small VPN IPSec hub-and-spoke entre Local 1, Local 2, Local 3 y AWS.} &
{\small NIST SP 800-53 SC-8\newline ISO/IEC 27001 A.13.2\newline \cite{nist80053,nist80077,iso27001}} &
{\small CE0312 -- Conectividad interlocal\newline CE0314 -- Redundancia VPN} &
{\small CUMPLE} \\ \hline

{\small T-09} &
{\small Dual-WAN con failover gestionado por pfSense.} &
{\small IEEE 802.3\newline ISO/IEC 27001 A.17\newline \cite{ieee80023,iso27001}} &
{\small CE0314 -- Redundancia WAN mediante Dual-ISP} &
{\small CUMPLE} \\ \hline

{\small T-10} &
{\small Routing inter-VLAN mediante pfSense y subinterfaces.} &
{\small IEEE 802.1Q\newline NIST SP 800-53 SC-7\newline \cite{ieee8021q,nist80053}} &
{\small CE0313 -- Inter-VLAN routing con pfSense} &
{\small CUMPLE} \\ \hline

{\small T-11} &
{\small Reglas base de segmentación entre VLANs.} &
{\small NIST SP 800-53 SC-7\newline ISO/IEC 27001 A.13.1\newline \cite{nist80053,iso27001}} &
{\small CE0313 -- Reglas base de segmentación} &
{\small CUMPLE} \\ \hline

{\small T-12} &
{\small WiFi empresarial con SSID asociados a VLANs.} &
{\small IEEE 802.11ac/ax} &
{\small CE0311 -- Requerimientos técnicos\newline CE0315 -- Elementos IEEE TO-BE} &
{\small TO-BE} \\ \hline

{\small T-13} &
{\small Convergencia rápida de capa 2 mediante RSTP.} &
{\small IEEE 802.1w} &
{\small CE0314 -- Restricciones y mejoras TO-BE\newline CE0315 -- Elementos IEEE TO-BE} &
{\small TO-BE} \\ \hline

{\small T-14} &
{\small Agregación de enlaces físicos mediante LACP.} &
{\small IEEE 802.3ad} &
{\small CE0314 -- Mejoras TO-BE\newline CE0315 -- Elementos IEEE TO-BE} &
{\small TO-BE} \\ \hline

{\small T-15} &
{\small Control de acceso por puerto integrado con identidad corporativa.} &
{\small IEEE 802.1X} &
{\small CE0311 -- Requerimientos técnicos\newline CE0315 -- Elementos IEEE TO-BE} &
{\small TO-BE} \\ \hline

{\small T-16} &
{\small Control de longitud máxima de canal para enlaces de cableado estructurado.} &
{\small TIA/EIA-568\newline ISO/IEC 11801} &
{\small CE0315 -- Cableado estructurado} &
{\small CUMPLE} \\ \hline

{\small T-17} &
{\small Direccionamiento IP privado y subnetting estructurado por sede y función.} &
{\small RFC 1918\newline IEEE 802.3\newline \cite{rfc1918,ieee80023}} &
{\small CE0313 -- Cálculos de subnetting} &
{\small CUMPLE} \\ \hline

{\small T-18} &
{\small Monitoreo de infraestructura y servicios críticos.} &
{\small ISO/IEC 27001 A.13.1\newline RFC 3411 / SNMP\newline \cite{iso27001,rfc3411}} &
{\small CE0311 -- Requerimientos técnicos\newline CE0314 -- Monitoreo y recuperación} &
{\small CUMPLE} \\ \hline

{\small T-19} &
{\small WAF basado en NGINX y ModSecurity ubicado en la DMZ.} &
{\small NIST SP 800-53 SC-7\newline OWASP Top 10\newline ISO/IEC 27001 A.13.1\newline \cite{nist80053,owasptop10,iso27001}} &
{\small CE0313 -- Diseño de la DMZ\newline CE0315 -- Seguridad de red} &
{\small CUMPLE} \\ \hline

{\small T-20} &
{\small Conectividad segura con AWS mediante VPN site-to-site.} &
{\small NIST SP 800-53 SC-8\newline ISO/IEC 27001 A.13.2\newline AWS Well-Architected\newline \cite{nist80053,iso27001,awswellarchitected}} &
{\small CE0312 -- Conectividad interlocal\newline CE0314 -- Redundancia VPN} &
{\small CUMPLE} \\ \hline

{\small T-21} &
{\small Backup a AWS RDS Multi-AZ + pg\_dump a S3 y respaldos lógicos programados.} &
{\small ISO/IEC 27001 A.17\newline NIST SP 800-34\newline \cite{iso27001,nist80034}} &
{\small CE0314 -- Redundancia de servicios críticos\newline CE0314 -- Plan de respaldo y recuperación} &
{\small CUMPLE} \\ \hline

{\small T-22} &
{\small Reinicio automático y recuperación controlada de microservicios del ERP.} &
{\small ISO/IEC 27001 A.17\newline NIST SP 800-53 CP-10\newline \cite{iso27001,nist80053}} &
{\small CE0314 -- Redundancia de servicios críticos} &
{\small CUMPLE} \\ \hline

{\small T-23} &
{\small Controladores de dominio y RODC para autenticación en sedes remotas.} &
{\small NIST SP 800-53 IA-2\newline ISO/IEC 27001 A.9\newline \cite{nist80053,iso27001}} &
{\small CE0312 -- Topología lógica por sede\newline CE0314 -- Servicios críticos} &
{\small CUMPLE} \\ \hline
...
La matriz muestra que la mayor parte de decisiones del diseño cuenta con respaldo normativo o técnico directo. Los elementos marcados como TO-BE no se consideran fallas del diseño, sino componentes que requieren condiciones de implementación más cercanas a un entorno productivo, como switches redundantes, autenticación por puerto, WiFi empresarial o agregación física de enlaces.

Esta trazabilidad permite sostener el CE0315 como una revisión normativa del diseño, no como una repetición de los bloques técnicos anteriores. La evidencia se concentra en relacionar cada decisión con su estándar correspondiente y en dejar explícito qué elementos pertenecen al alcance actual y cuáles forman parte de la evolución propuesta.


% ============================================================
%  ANEXO I — Diseño detallado de VLANs por sede
% ============================================================
\chapter{Anexo~I --- Diseño Detallado de VLANs por Sede}
\label{anex:vlans_detalle}

El presente anexo documenta la segmentación de red completa de cada sede de Franco's SAC, especificando la función, subred, gateway y criterio de aislamiento de cada VLAN. Este nivel de detalle complementa el resumen general de la sección CE0313 y sirve como referencia de implementación para la configuración de pfSense y los vSwitches ESXi.

\section*{I.1 \ Sede Central --- Local 2}

La sede central concentra los segmentos más críticos. Incorpora 7 VLANs: Administración, Usuarios, DMZ, Servicios, Datos, Gestión y Management ESXi. Las subredes \textbf{/24} cubren zonas con mayor densidad de hosts (VLAN 10, 20, 40); las subredes \textbf{/28} minimizan la superficie de exposición en las zonas más sensibles (VLAN 30, 50, 60, 99).
...
\begin{xltabular}{\linewidth}{@{}|N{1.4cm}|L{2.8cm}|L{2.8cm}|L{2.4cm}|L{4.2cm}|X|@{}}
\caption{Diseño de VLANs de la sede central}
\label{tab:vlans_sede_central} \\

\arrayrulecolor{upeu_dark}
\rowcolor{upeu_dark}
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small VLAN} &
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small Zona lógica} &
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small Subred} &
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small Gateway} &
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small Componentes principales} &
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small Criterio de segmentación} \\ \hline
\endfirsthead
...
{\small 10.10.10.0/24} &
{\small 10.10.10.1} &
{\small Active Directory, DNS, NTP y servicios administrativos internos.} &
{\small Separa los servicios de identidad y administración de las redes de usuarios y de las zonas expuestas. Esta VLAN debe tener reglas controladas, ya que participa en autenticación y resolución de nombres.} \\ \hline

{\small 20} &
{\small Usuarios} &
{\small 10.10.20.0/24} &
{\small 10.10.20.1} &
{\small Estaciones de trabajo, usuarios corporativos y acceso interno al ERP.} &
{\small Agrupa el tráfico de usuarios finales. Desde esta VLAN se permite el acceso al ERP mediante la DMZ, pero se restringe la comunicación directa hacia microservicios y bases de datos.} \\ \hline

{\small 30} &
{\small DMZ} &
{\small 10.10.30.0/28} &
{\small 10.10.30.1} &
{\small WAF, API Gateway y componente de contingencia del API Gateway.} &
{\small Funciona como zona intermedia para servicios publicados. No debe tener acceso directo a la capa de datos; su comunicación hacia servicios internos debe ser limitada y justificada.} \\ \hline

{\small 40} &
{\small Servicios} &
{\small 10.10.40.0/24} &
{\small 10.10.40.1} &
{\small Microservicios del ERP, broker NATS, servidor de archivos y servicios internos.} &
{\small Aloja la lógica de negocio y servicios internos. Solo debe recibir tráfico desde componentes autorizados, como el API Gateway, y comunicarse con bases de datos específicas cuando sea necesario.} \\ \hline

{\small 50} &
{\small Datos} &
{\small 10.10.50.0/28} &
{\small 10.10.50.1} &
{\small Bases de datos PostgreSQL asociadas a los módulos del ERP.} &
{\small Se considera una zona crítica. El acceso debe permitirse únicamente desde microservicios autorizados, evitando conexiones directas desde usuarios, DMZ o redes externas.} \\ \hline

{\small 60} &
{\small Gestión} &
{\small 10.10.60.0/28} &
{\small 10.10.60.1} &
{\small Monitoreo híbrido: Wazuh Manager+Indexer+Dashboard (10.10.60.10), agentes Wazuh en todas las VMs, AWS CloudWatch, SNMP, ICMP.} &
{\small Centraliza la observabilidad de la infraestructura. Su acceso debe estar reservado al personal técnico, ya que permite consultar el estado de servidores, servicios y dispositivos de red.} \\ \hline

{\small 99} &
{\small Management ESXi} &
{\small 10.10.99.0/28} &
{\small 10.10.99.1} &
{\small Interfaces de administración de hipervisores ESXi, VMkernel y gestión de plataforma virtual.} &
{\small Aísla la administración de virtualización del tráfico operativo. Esta VLAN no debe ser accesible desde redes de usuarios ni desde servicios publicados.} \\ \hline
...
\section*{I.2 \ Sedes Remotas --- Local 1 y Local 3}

Las sedes remotas mantienen 5 VLANs cada una (10, 20, 40, 60, 99), suprimiendo la DMZ y la VLAN de Datos que permanecen centralizadas en Local~2. Local~1 (H4) y Local~3 (H5) operan como nodos livianos conectados mediante VPN hub-and-spoke.

\subsection*{I.2.1 \ VLANs del Local 1}
...
\begin{xltabular}{\textwidth}{@{}|N{1.3cm}|L{3cm}|L{3.2cm}|X|@{}}
\caption{Diseño de VLANs del Local 1 -- Planta de Producción 1}
\label{tab:vlans_local_1} \\

\arrayrulecolor{upeu_dark}
\rowcolor{upeu_dark}
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small VLAN} &
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small Zona lógica} &
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small Subred / gateway} &
\parbox[c][1.1cm][c]{\linewidth}{\centering\color{white}\bfseries\small Función dentro de la sede remota} \\ \hline
\endfirsthead
...
{\small 10} &
{\small Administración} &
{\small 10.20.10.0/24\newline GW: 10.20.10.1} &
{\small RODC DC3 (Samba, 10.20.10.20) + servicios de identidad locales. Autentica usuarios de la planta sin necesidad de VPN activa.} \\ \hline

{\small 20} &
{\small Usuarios} &
{\small 10.20.20.0/24\newline GW: 10.20.20.1} &
{\small Estaciones de trabajo y usuarios operativos de producción. Acceso al ERP vía VPN L1-L2 (HTTPS 443 hacia API-GW).} \\ \hline

{\small 40} &
{\small Servicios / FileServer} &
{\small 10.20.40.0/24\newline GW: 10.20.40.1} &
{\small FileServer-L1 (10.20.40.10) para almacenamiento local de la planta. VLAN 40 replicada desde L2 por coherencia estructural y principio de mínima divergencia de diseño entre sedes.} \\ \hline

{\small 60} &
{\small Gestión / Wazuh} &
{\small 10.20.60.0/28\newline GW: 10.20.60.1} &
{\small Agentes Wazuh instalados en las VMs de H4. Reportan al Wazuh Manager en L2 (10.10.60.10) vía VPN. SNMP polling local también en este segmento.} \\ \hline

{\small 99} &
{\small Management ESXi} &
{\small 10.20.99.0/28\newline GW: 10.20.99.1} &
{\small Administración exclusiva del hipervisor ESXi H4. Aislado del tráfico operativo; solo accesible para personal técnico autorizado.} \\ \hline

\end{xltabular}

\normalsize

\subsection*{I.2.2 \ VLANs del Local 3}
...
\begin{xltabular}{\textwidth}{@{}|N{1.3cm}|L{3cm}|L{3.2cm}|X|@{}}
\caption{Diseño de VLANs del Local 3 -- Segunda Planta de Producción}
\label{tab:vlans_local_3} \\
...
{\small 10} &
{\small Administración} &
{\small 10.30.10.0/24\newline GW: 10.30.10.1} &
{\small RODC DC4 (Samba, 10.30.10.20) + servicios de identidad locales. Autentica usuarios de la 2ª planta sin dependencia de VPN activa.} \\ \hline

{\small 20} &
{\small Usuarios} &
{\small 10.30.20.0/24\newline GW: 10.30.20.1} &
{\small Estaciones de trabajo y usuarios operativos de produccion bajo estimacion preliminar. Acceso al ERP via VPN L2-L3 (HTTPS 443 hacia API-GW).} \\ \hline

{\small 40} &
{\small Servicios / FileServer} &
{\small 10.30.40.0/24\newline GW: 10.30.40.1} &
{\small FileServer-L3 (10.30.40.10) para almacenamiento local de la 2a planta. Misma estructura que Local 1 para coherencia de diseño.} \\ \hline

{\small 60} &
{\small Gestión / Wazuh} &
{\small 10.30.60.0/28\newline GW: 10.30.60.1} &
{\small Agentes Wazuh en VMs de H5. Reportan al Wazuh Manager en L2 (10.10.60.10) via VPN. Supervision tecnica local documentada como parte del entorno de laboratorio.} \\ \hline

{\small 99} &
{\small Management ESXi} &
{\small 10.30.99.0/28\newline GW: 10.30.99.1} &
{\small Administración exclusiva del hipervisor ESXi H5. Aislado del tráfico operativo; solo accesible para personal técnico autorizado.} \\ \hline
